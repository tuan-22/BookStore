 ...
 * @author tramy
 */
public class ImReceiptDetailController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            List<ReceiptDetail> list = dao.getListRDetailByRID(id);
            ReceiptDAO receiptDAO = new ReceiptDAO();
            Receipt receipt = receiptDAO.getReceiptByid(id);

            request.setAttribute("listD", list);
            request.setAttribute("receipt", receipt);
            request.getRequestDispatcher("impReceiptDetail.jsp").forward(request, response);
        }
    }
